...
class FluidState(object):
    ...
    def update_ph(self, p_bar, h):
        ...
        try:
            ...
            raise ValueError("h {:.0f}J/kg p{:.2f}bar fluid ".format(self.s, self.p)+self.fluid)
...
class Compressor(TwoStationComponent):
    ...
    def compute(self, pOut, eta=1, pressureRatio=None, forward=True):
        if pOut == None and pressureRatio != None:
            pOut = self.inlet.state.p * pressureRatio
        elif pOut == None and pressureRatio == None:
            raise ValueError("No compression data passed to compressor")
        ...
        if forward:
            ...
            self.inlet.flow.mDot = self.outlet.flow.mDot
...
class Turbine(TwoStationComponent):
    ...
    def compute(self, pOut, eta=1, forward=True):
        if pOut > self.inlet.state.p:
...
class Heater(TwoStationComponent):
    ...
    def compute(self, TOut=None, QIn=None, forward=True):
        if self.mode == "T":
            if TOut != None:
...
class Cooler(TwoStationComponent):
    ...
    def compute(self, TOut=None, QOut=None, forward=True):
        if self.mode == "T":
            if TOut != None:
...
class Recuperator(CycleComponent):
    ...
    def compute(self, eta=1, forward=True):
...
class IdealThrottle(TwoStationComponent):
    ...
    def compute(self, pOut, forward=True):
        if pOut > self.inlet.state.p:
...
class PhaseSeperator(CycleComponent):
    ...
    def compute(self, forward=True):
        ...
        if forward:
            ...
            self.inlet.flow.mDot = self.liquidOutlet.flow.mDot + self.gasOutlet.flow.mDot
...
class CompressibleOrifice(TwoStationComponent):
    def __init__(self):
        super().__init__()
        self.compType = "Compressible Orifice"
    
    def compute(self, CdA, forward=True):
        pUp = self.inlet.state.p * 1e5
        pDown = self.outlet.state.p * 1e5
        TUp = self.inlet.state.T

        if pDown > pUp:
            raise ValueError("Orifice outlet pressure greater than inlet pressure")

        
        gamma = PropsSI('ISENTROPIC_EXPANSION_COEFFICIENT','P',pUp,'T',TUp,self.inlet.state.fluid)
        gas_constant = PropsSI('GAS_CONSTANT','P',pUp,'T',TUp, self.inlet.state.fluid)
        R = (gas_constant / PropsSI('MOLARMASS','P',pUp,'T',TUp ,self.inlet.state.fluid)) #J/kgK
        gc = 1

        pCrit = pUp * ((2 / (gamma + 1)) ** (gamma / (gamma - 1)))
        ...
        if pDown > pCrit:
            ...
            ((pDown/pUp)**(1/gamma)) *
            np.sqrt(
                (2 * gc * R * TUp * (gamma / (gamma-1))) *
                (1 - ((pDown/pUp)**((gamma - 1)/gamma)))
            ))
        else:
            flowrate = CdA * (
                pUp *
        ...
        if forward:
            self.outlet.flow.mDot = flowrate
        else:
            self.inlet.flow.mDot = flowrate
        
        #print(pCrit, gamma, R, flowrate, pUp, pDown)

        self.outlet.state.update_ph(self.outlet.state.p, self.inlet.state.h)
