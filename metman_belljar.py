...
def makeSolverBodyParams(height_cm, weight_kg, varyMetabolism=True, useNavyBodyFat=True, 
    ...
    metabolismAreaFactor = 45.11    # W/m2
    ...
    for zone in zones:
        ...
        print(zone, zoneAreas[zone])

    totalMetabolism = 0
    for z in thermalNodeValues:
        totalMetabolism += thermalNodeValues[z]["Basal metabolism"]
    print("BASAL: ", totalMetabolism)
...
def calculateSpacesuitInternalThermalConnections(solver, 
                                       ...
                                       externalIsVacuum=True, externalT=250, externalEmissivity=0.8, createExternal=False):
    ...
    for zone in zones:
        ...
        if createExternal:
            solver.components[zone+"/Suit | Environment"] = bjls.RadiationTransfer(areaInlet=solver.controlParameters["Suit areas"][zone],
                                                                                areaOutlet=1, 
            ...
            solver.connectStationsSolid(zone+"/Suit", "outlet", zone+"/Suit | Environment", "inlet", 
                                        mass=solver.controlParameters["Suit masses"][zone]/2, specificHeatCapacity = suitSHC)
...
def calculateSpacesuitExternalThermalConnections(solver, suitSHC,
                                                 groundTemp, groundViewFactor,
                                                 skyTemp,
                                                 sunAltitude, sunIntensity,
                                                 isVacuum=True, airTemp=298):
    ...
    for zone in zones:
        ...
        solver.components[zone+"/Suit | Ground"] = bjls.RadiationTransfer(
            areaInlet=solver.controlParameters["Suit areas"][zone], 
            emittanceInlet=solver.controlParameters["Suit outer emissivity"], 
...
def systemController(self, time, dt):
    ...
    shiveringHeat = max(shiveringHeat, 0)
...
def initialiseSpacesuit(solver, TEnvironment=250, TAir=290, TLCG=290):
    ...
    TInner = 290#min(TAir - 25, THalf)
    TOuter = 290#max(TEnvironment+25, THalf)
    if TLCG != TAir:
        TSkin = min(TLCG, 309)
    else:
...
solver = bjls.TimestepSolver(maxTimestep=60, precision=3)
...
solver.controlParameters = {
    ...
    "Suit areas": {
        "Head":   0.15, "Trunk":  0.80,
        "Arm L":  0.15, "Arm R":  0.15,
        "Hand L": 0.06, "Hand R": 0.06,
        "Leg L":  0.40, "Leg R":  0.40,
        "Foot L": 0.10, "Foot R": 0.10},
    "Suit masses": {
        "Head":   1.00, "Trunk":  2.50,
        "Arm L":  0.75, "Arm R":  0.75,
        "Hand L": 0.75, "Hand R": 0.25,
        "Leg L":  1.00, "Leg R":  1.00,
        "Foot L": 0.25, "Foot R": 0.25}, 
    "Set points": {
        "Core": {
...
thermalNodeValues = makeSolverBodyParams(175, 70, useNavyBodyFat=False)
...
calculateSpacesuitInternalThermalConnections(solver,
                                     suitPressureBar=0.3, TInlet=300, approxTGarment=295, 
                                     inletHumidity=0.05, inletCO2=400e-6, 
                                     undergarmentEmissivity=0.8, suitInnerEmissivity=0.8, suitOuterEmissivity=0.5, 
                                     suitOuterAbsorbivity=0.5, suitSHC=1000, 
                                     LCGTInlet=295, useLCG=False,
                                     undergarmentConductivities={
                                        "Head":   0, "Trunk":  0.005,
                                        "Arm L":  0.005, "Arm R":  0.005,
                                        "Hand L": 0, "Hand R": 0,
                                    ...
                                    LCGFlowRates={
                                        "Head":   0, "Trunk":  0.0132,
                                        "Arm L":  0.00255, "Arm R":  0.00255,
                                        "Hand L": 0, "Hand R": 0,
                                        "Leg L":  0.00585, "Leg R":  0.00585,
                                        "Foot L": 0, "Foot R": 0},
                                    suitConductivities={
                                        "Head":   07.8, "Trunk":  40.0,
                                        "Arm L":  06.7, "Arm R":  06.7,
                                        "Hand L": 02.8, "Hand R": 02.8,
                                        "Leg L":  17.6, "Leg R":  17.6,
                                        "Foot L": 03.8, "Foot R": 03.8}, 
                                    zoneAirflows={
                                        "Air inlet": 0.0012,
                                        "Head":   0.0012, "Trunk":  0.0012,
                                        "Arm L":  0.0003, "Arm R":  0.0003,
                                        "Hand L": 0.0003, "Hand R": 0.0003,
                                        "Leg L":  0.0003, "Leg R":  0.0003,
                                        "Foot L": 0.0003, "Foot R": 0.0003}, 
                                    zoneAirConnectivity={
...
calculateSpacesuitExternalThermalConnections(solver, 1000, 250, 0.5, 200, 80, 610)
...
start = time.time()
solver.runSolver(0, 4.5*60*60, 1)
...
ax1a.plot(solver.outputHistory["Time (min)"], solver.outputHistory["Q sensible (W)"], label="Q sensible")
ax1a.plot(solver.outputHistory["Time (min)"], solver.outputHistory["Q LCG (W)"], label="Q LCG", linestyle="--")
ax1a.plot(solver.outputHistory["Time (min)"], solver.outputHistory["Heat storage rate (W)"], label="Q stor", linestyle=":")
ax1a.plot(solver.outputHistory["Time (min)"], solver.outputHistory["Q latent (W)"], label="Q latent")

ax1a.hlines(0, 0, max(solver.outputHistory["Time (min)"]), colors=["k"], linewidths=[0.5])

ax1a.set_ylim(-500, 500)
...
ax2a.plot(solver.outputHistory["Time (min)"], solver.outputHistory["Total storage (BTU)"], label="E store", linestyle=":", c="k")
...
ax1a.set_ylabel("Heat transfer (W)")
ax2a.set_ylabel("Stored energy (BTU)")
ax2a.set_xlabel("Time (min)")
ax2a.set_title("Heat transfer and stored energy - thin suit low airflow")

figb, ax1b = plt.subplots()
ax2b = ax1b.twinx()

ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Metabolism @ Trunk/Skin"].outlet.state.THistory], label="Trunk Skin", c="red")
ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Trunk/Suit"].inlet.state.THistory], label="Trunk Suit Inside", c="red", linestyle="--")
ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Trunk/Suit"].outlet.state.THistory], label="Trunk Suit Outside", c="red", linestyle=":")

ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Metabolism @ Arm L/Skin"].outlet.state.THistory], label="Arm Skin", c="blue")
ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Arm L/Suit"].inlet.state.THistory], label="Arm Suit Inside", c="blue", linestyle="--")
ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Arm L/Suit"].outlet.state.THistory], label="Arm Suit Outside", c="blue", linestyle=":")

ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Metabolism @ Hand L/Skin"].outlet.state.THistory], label="Hand Skin", c="green")
ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Hand L/Suit"].inlet.state.THistory], label="Hand Suit Inside", c="green", linestyle="--")
ax1b.plot(solver.outputHistory["Time (min)"], [i-273 for i in solver.components["Hand L/Suit"].outlet.state.THistory], label="Hand Suit Outside", c="green", linestyle=":")

ax1b.legend()

ax1b.set_xlabel("Time (min)")
ax1b.set_ylabel("Temperature (deg C)")
ax2b.set_ylabel("Temperature (deg F)")
ax1b.set_title("Skin and suit temperatures - thin suit low airflow")
ax1b.set_ylim(-10, 40)
ax2b.set_ylim(14, 104)

#plt.yticks(np.arange(310.15, 330.15), [str(int(i)) for i in np.arange(310.15-273.15, 330.15-273.15)])


#np.savetxt('data1.csv', [p for p in zip(solver.outputHistory["Time (min)"][::10], 
#                                        solver.outputHistory["Total storage (J)"][::10], 
#                                        solver.outputHistory["Temp skin average (K)"][::10], 
#                                        solver.outputHistory["Temp head core (K)"][::10])], delimiter=',')


data = ["Total storage (J)", "Temp skin average (K)", "Temp head core (K)", "Q shiver (W)", "Q latent (W)"]

print(solver.components["Trunk/Suit | Sky"].radiationHeatTransfer)
print(solver.components["Trunk/Suit | Ground"].radiationHeatTransfer)

for i_t10, t in enumerate(solver.outputHistory["Time (min)"][::80]):
    i_t = i_t10 * 80
    datapnts = ["{:.3f}".format(solver.outputHistory[j][i_t]) for j in data]
    strout = "{:.3f},".format(t)
    for d in datapnts:
        strout += d
        strout += ","
...
# popt, pcov = curve_fit(func, timeRanges[0], storageRanges[0], method="lm")
# print(popt)
# print(np.diag(pcov))

# ax2.hlines(popt[2], [0], [solver.outputHistory["Time (min)"][-1]], linestyles="dashed", colors=["k"])

#print(solver.outputHistory["Time (min)"][::10])
#print("")
#print(solver.outputHistory["Total storage (J)"][::10])
#print("")
#print(solver.outputHistory["Temp skin average (K)"][::10])
#print("")
#print(solver.outputHistory["Temp head core (K)"][::10])
